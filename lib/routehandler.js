...
routeHandler._returnBook.post = (data, callback) => {
    const token = typeof data.payload.token == 'string' && data.payload.token.length == 256 ? data.payload.token : null;
    
    if (token) {
        const user = findUserByToken(token);
        if (user.data.book) {
            const book = user.data.book;
            delete user.data.book;
            updateBookAmount('inc', book, err => {
                if (!err) {
                    fs.writeFile(path.join('users', user.userfile), JSON.stringify(user.data), err => {
                        if (!err) {
            ...
            callback(400, {message: "User has no book to return"})
        }
    } else {
...
routeHandler._books.post = (data, callback) => {
    ...
    if(name && price && author && publisher && amount && token){
        ...
        if (user.data.isAdmin == false) {
            ...
            const writableData = { ...data.payload };
            delete writableData.token;
            fileUtil.create('books', fileName, writableData, (err) => {
                if (!err) {
