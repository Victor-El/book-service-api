...
routeHandler._books.post = (data, callback) => {
    ...
    console.log(name, price, author, publisher);
...
routeHandler._books.delete = (data, callback) => {
 ...
 routeHandler.CreateAdmin = (data, callback) => {
     const acceptableHeaders = ["post"];
     if (acceptableHeaders.indexOf(data.method) > -1) {
         routeHandler._createAdmin[data.method](data, callback);
     } else {
         callback(405);
     }
 };
...
routeHandler.Admin = (data, callback) => {
    const acceptableHeaders = ["post", "get", "put", "delete"];
    if (acceptableHeaders.indexOf(data.method) > -1) {
        routeHandler._admin[data.method](data, callback);
    } else {
        callback(405);
    }
};
...
routeHandler._admin = {};
routeHandler._createAdmin = {};

routeHandler._createAdmin.post = (data, callback) => {
    if (data.payload.username && data.payload.password) {
        fileUtil.create("admins", data.payload.username, data.payload, err => {
            if (!err) {
                callback(200, {message: "admin created successfully"});
            } else {
...
routeHandler._admin.post = (data, callback) => {
